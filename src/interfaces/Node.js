...
class Node {
	...
	* @returns {node} The file node
	 ...
	 */
	exists() {
		return this.access(this.fs.constants.F_OK).then(() => {
			return true;
		}).catch(() => false);
	}
   ...
   *    ]
	...
	static __normalizeGlobOptions(options = {}) {
		...
		if ( type !== 'undefined' && ['string', 'object'].indexOf(type) === -1 ) {
			throw new Error(
				'Optional `options` parameter must be either a string or an object!'
			);
		}
	...
	__normalizeRelativeGlobOptions(pattern, options = {}) {
		...
		const dirPath = this.nodeName === 'Directory'
			? this.pathName
			: this.parentPath;
	...
	__resolvePath(dir) {
		...
		if ( !isDirectory && dirType !== 'string' ) {
			throw new Error(
				'`dir` parameter must be a string or instance of Directory class!'
			);
		}
